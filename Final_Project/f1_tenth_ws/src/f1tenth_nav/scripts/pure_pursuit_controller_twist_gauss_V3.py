...
class pure_pursuit_twist():
	...
	gl_array = laser_data#np.convolve(laser_data,gmask,mode="same")

	der_mask = np.array([1, -1, 0])
	dgl_arr = np.convolve(gl_array,der_mask,mode="same")
	...
	if np.sum(dgl_arr > tolerance)> 0.0:
		...
		thresh = dgl_arr < -tolerance
		dtemp = thresh*dgl_arr
		ind = np.argwhere(dtemp == np.min(dtemp))[0][0]+1
		x1 = -0.16#x_coor[len(laser_data)-1]
